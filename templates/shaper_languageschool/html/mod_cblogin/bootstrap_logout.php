...
<form action="<?php echo $_CB_framework->viewUrl( 'logout', true, null, 'html', $secureForm ); ?>" method="post" id="login-form" class="form-vertical cbLogoutForm">
	...
	<?php if ( (int) $params->get( 'greeting', 1 ) ) { ?>
		<div class="mb-2 login-greeting <?php echo htmlspecialchars( $templateClass ); ?>">
			<?php echo $greetingText; ?>
		</div>
	<?php } ?>
	<?php
		$_afk_lang = \Joomla\CMS\Factory::getLanguage()->getTag();
		$_afk_label = strpos($_afk_lang,'zh') !== false ? '我的会员空间'
		            : (strpos($_afk_lang,'en') !== false ? 'My Member Space' : 'Mon espace inscrit');
	?>
	<div class="afk-cb-profile-link">
		<a href="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_comprofiler&view=userprofile&Itemid=1046'); ?>" class="btn btn-sm btn-outline-danger w-100 mb-2">
			<span class="fa fa-user"></span> <?php echo htmlspecialchars($_afk_label); ?>
		</a>
	</div>
